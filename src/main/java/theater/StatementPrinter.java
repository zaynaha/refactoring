...
public class StatementPrinter {
    ...
    public String statement() {
        ...
        for (Performance performance : invoice.getPerformances()) {
            ...
            result.append(String.format("  %s: %s (%s seats)%n", getPlay(performance).getName(),
                    format.format(thisAmount / Constants.PERCENT_FACTOR), performance.getAudience()));
    ...
    private static int getAmount(Performance performance) {
        ...
        switch (getPlay(performance).getType()) {
            case "tragedy":
                typeAmount = Constants.TRAGEDY_BASE_AMOUNT;
                if (performance.getAudience() > Constants.TRAGEDY_AUDIENCE_THRESHOLD) {
                    typeAmount += Constants.TRAGEDY_OVER_BASE_CAPACITY_PER_PERSON
                            * (performance.getAudience() - Constants.TRAGEDY_AUDIENCE_THRESHOLD);
                }
                break;
            case "comedy":
                typeAmount = Constants.COMEDY_BASE_AMOUNT;
                if (performance.getAudience() > Constants.COMEDY_AUDIENCE_THRESHOLD) {
                    typeAmount += Constants.COMEDY_OVER_BASE_CAPACITY_AMOUNT
                            + (Constants.COMEDY_OVER_BASE_CAPACITY_PER_PERSON
                            * (performance.getAudience() - Constants.COMEDY_AUDIENCE_THRESHOLD));
                }
                typeAmount += Constants.COMEDY_AMOUNT_PER_AUDIENCE * performance.getAudience();
                break;
            default:
